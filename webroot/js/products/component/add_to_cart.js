export const add_to_cart = {
    ...
    methods: {
        addToCart (d1,d2,d3,d4,d5,d6,d7,d8) {
            ...
            if(itemInCart){
                let cartPreorder = (d7 && itemInCart[0].po === 1 || !d7 && itemInCart[0].po === 2 || !d7 && itemInCart[0].po === 0) ? true : false
                if(cartPreorder) {
                    this.payloadToStore(d1,d2,d3,d4,d5,d6,d7,d8)
                }else{
                    let typeInCart = (itemInCart[0].po === 1) ? 1 : (itemInCart[0].po === 0 || itemInCart[0].po === 2) ? 0 : null
